...
HAL_StatusTypeDef read_data(uint8_t *data, uint8_t data_size) {
	uint8_t buffer_size = data_size + 10;
	uint8_t buffer[buffer_size];
	uint8_t offset = 0;

	// HAL_StatusTypeDef status = HAL_BUSY;
	// while (status == HAL_BUSY) {
	HAL_UART_Receive(&huart4, buffer, buffer_size, R_TIME_OUT);
	...
	for (uint8_t i = 0; i < data_size; i++)
		data[i] = buffer[i + 7 + offset];

	uint8_t ck1 = checksum1(data, data_size, buffer[3 + offset], buffer[4 + offset]); // 6. Checksum1
	uint8_t ck2 = checksum2(ck1);				  // 7. Checksum2

	if (ck1 != buffer[5 + offset])
		return HAL_ERROR;
	if (ck2 != buffer[6 + offset])
		return HAL_ERROR;
